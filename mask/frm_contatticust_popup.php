...
<?php
  ...
  define("FILE_CORRENTE", "frm_contatticust_popup.php");
  ...
  if ($cmd['comando']==FASEINS) {
    //idc
    $editval['idcustomer']=$_REQUEST['idc'];
  }
  
  if ($selobj!=0) {
    $query = "SELECT id, idcustomer, nome, posizione, tel_lavoro, tel_privato, fax, mobile, skype, mail1, note
  ...
  print "<BR><H1>"._CUSTOMERCONTATTI_."</H1>";
  
  
  print "<BR>";
  
  if ($cmd['comando']==FASEMOD) {
      $titolofinestra = _MODCUSTOMERCONTATTI_; $classtitolo = "headermod";
      //echo "XXX";
  } else {
      $cmd['comando']=FASEINS;
      $titolofinestra = _INSCUSTOMERCONTATTI_; $classtitolo = "headerins";
      //echo "<BR> xxcmd ".$cmd;
  }
  ...
  $query = "SELECT id, nome, apellido, ragsoc, fantasia
            FROM customer
            WHERE id=".$editval['idcustomer'];
  $result = mysql_query ($query) or die ("Error_1.2");
  $line = mysql_fetch_assoc($result);
  
  print "<TR>";
  print "<TD CLASS='form_lbl'>"._LBLCUSTOMER_."</TD>";
  print "<TD><INPUT TYPE='hidden' size='50' NAME='idcustomer' VALUE='". $editval['idcustomer'] ."'><INPUT TYPE='text' size='50' NAME='lblidcustomer' VALUE='". $line ['apellido']." ".$line['nome']." - ".$line['ragsoc']."' READONLY></TD>";
  print "<TD CLASS=form_help></TD>";
  print "</TR>";
