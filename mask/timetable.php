...
//ricalcolo datai per lunedi' 1Ý giorno
    ...
    */
    ...
    if ($line['fdel']==1) {
      ...
      if ($line['fsost']==1) {
        ...
        //attivo
        //print "A<HR>$chiave2<HR>";
        //print "B<HR>".$appvetelement['durata']."<HR>";
        for ($irelo = $chiave2;$irelo<($chiave2 + ($appvetelement['durata']*60));$irelo+=$tt_step) {
          ...
          if (isset($lezioni[$chiave][$irelo])) {
            ...
            //print "<HR>PRE<HR>";
            //myprint_r($lezioni[$chiave][$chiave2]);
            //reloco
            foreach ($lezioni[$chiave][$irelo]['fdel_1'] as $keyrelo=>$currelo) {
              $lezioni[$chiave][$chiave2]['ne_fdel_1']++;
              $lezioni[$chiave][$chiave2]['fdel_1'][$lezioni[$chiave][$chiave2]['ne_fdel_1']] = $currelo;
            }
            foreach ($lezioni[$chiave][$irelo]['fsost_1'] as $keyrelo=>$currelo) {
              $lezioni[$chiave][$chiave2]['ne_fsost_1']++;
              $lezioni[$chiave][$chiave2]['fsost_1'][$lezioni[$chiave][$chiave2]['ne_fsost_1']] = $currelo;
            }
            //print "<HR>DOPO<HR>";
            //myprint_r($lezioni[$chiave][$chiave2]);
          ...
          for ($irelo = $chiave2;$irelo<($chiave2 + ($appvetelement['durata']*60));$irelo+=$tt_step) {
            if (isset($lezioni[$chiave][$irelo])) {
              ...
              for($jrelo=1;$jrelo<=$lezioni[$chiave][$irelo]['ne_fdel_1'];$jrelo++) {
                ...
                unset($lezioni[$chiave][$irelo]['fdel_1'][$jrelo]);
              }
    ...
    for ($j=0;$j<$numerogg*$daysweek;$j++) {
      ...
      if ($tt_noday[$appwkday]!=1) {      
        ...
        if (isset($lezioni[$chiave][$ora_prec])) {
          //l'evento esiste (pu= essere cancellato, sostituito o attivo)
